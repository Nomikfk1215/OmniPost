...
import { extractImagePositions, normalizeImageAssets } from "@/lib/images/assets";
...
- 原文中出现的正文图片是内联图片，平台适配时必须保留在相邻段落附近；不要把正文图片只当封面或素材池。
- imageSuggestions 写“适合放进图片里的关键句、总结句或疑问句”，不要写“封面风格、背景色、构图、3:4、插画风格”这类视觉提示词。
...
export function buildPrompt(params: {
  ...
  const imagePositions = extractImagePositions(params.content.rawText);
  const positionsByUrl = new Map(imagePositions.map((position) => [position.url, position]));
  const inlineImages = imageAssets
    .filter((image) => image.source === "markdown")
    .map((image) => ({
      ...image,
      position: positionsByUrl.get(image.url) ?? null
    }));
  const standaloneImages = imageAssets.filter((image) => image.source !== "markdown");
  const imageSections = [
    inlineImages.length
      ? `正文内联图片（已在原文中占位，适配时应保留在对应段落附近；HTML 平台用 <img>，文本平台用图片占位说明，不要改写 url）：\n${JSON.stringify(inlineImages, null, 2)}`
      : "",
    standaloneImages.length
      ? `可用封面/图库图片资产（可用于封面、多图或额外配图，返回内容时不要改写 url）：\n${JSON.stringify(standaloneImages, null, 2)}`
      : ""
  ].filter(Boolean);
  const userMessage = [
...
5. imageSuggestions 只输出图片上可直接展示的文案，如关键判断、总结性问题、核心句；不要输出图片制作说明。`
  ]
    .filter(Boolean)
    .join("\n\n");
